...
async function run() {
  ...
  try {
    ...
    await page.getByText('История подключений аккаунтов',{exact:true}).click()
    ...
    await openLinkedInManual(page, 303)
    await page.getByTestId('linkedin-manual-303').getByTestId('linkedin-error-code').waitFor()
    ...
    await page.getByTestId('linkedin-manual-103').getByText('Completed', { exact: true })
      .waitFor({ timeout: 5000 })
    await page.getByTestId('linkedin-history-view').click()
    await page.getByText('История подключений аккаунтов',{exact:true}).click()
