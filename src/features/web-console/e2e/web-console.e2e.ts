...
async function runTests(): Promise<void> {
  ...
  const backend = spawnProcess(process.execPath, ['src/features/web-console/backend/index.ts'], {
    ...
    WEB_CONSOLE_PLATFORM_ACCOUNT_POLICY_EMAILS: 'client@example.com',
  ...
  try {
    ...
    await page.getByTestId('client-dashboard').waitFor()
    await page.getByTestId('open-account-editor-button').click()
    const legacyOptions = (await page.getByTestId('account-platform').locator('option').allTextContents())
      .map((label: string) => label.trim())
    assert.equal(legacyOptions.includes('whatsapp'), true)
    assert.equal(legacyOptions.filter((label: string) => label === 'email_ru').length, 2)
    assert.equal(await page.getByTestId('account-label').isDisabled(), false)
    assert.equal(await page.getByTestId('account-login').isDisabled(), false)
    assert.equal(await page.getByTestId('account-phone').isDisabled(), false)
    await page.getByTestId('account-form').getByText('LinkedIn URL', { exact: true }).waitFor()
    await page.getByTestId('close-account-editor-button').click()
    await page.getByTestId('edit-account-button').first().click()
    await page.getByTestId('account-platform').waitFor()
    assert.equal(await page.getByTestId('account-platform-locked').count(), 0)
    assert.equal(await page.getByTestId('account-label').isDisabled(), false)
    await page.getByTestId('close-account-editor-button').click()
    await page.getByTestId('logout-button').click()
    await page.getByTestId('login-page').waitFor()
    ...
    await page.getByTestId('login-button').click()
    await page.getByTestId('client-dashboard').waitFor()
    await assertText(page, 'Аблена Дементьева')
    await page.getByTestId('open-account-editor-button').click()
    const targetOptions = (await page.getByTestId('account-platform').locator('option').allTextContents())
      .map((label: string) => label.trim())
    assert.equal(targetOptions.includes('whatsapp'), false)
    assert.equal(targetOptions.filter((label: string) => label === 'email_ru').length, 1)
    ...
    const providerInitialStatusRequests: string[] = []
    const captureProviderInitialStatusRequest = (request: any) => {
      if (request.url().includes('/api/dolphin/profiles/status')) providerInitialStatusRequests.push(request.url())
    }
    page.on('request', captureProviderInitialStatusRequest)
    ...
    assert.deepEqual(providerInitialStatusRequests, [], 'provider dashboard must not issue per-client status requests')
    page.off('request', captureProviderInitialStatusRequest)
    ...
    assert.equal(await page.getByTestId('open-dolphin-provider-button').count(), 3)
    ...
    await page.getByTestId('provider-clients-table').locator('tbody tr').first().getByTestId('provider-details-button').click()
    const providerDrawer = page.getByTestId('provider-details-drawer')
    await providerDrawer.waitFor()
    ...
    await providerDrawer.getByText('Smartcat, Bank CenterCredit', { exact: false }).waitFor()
    assert.equal(await providerDrawer.getByText('Wrong response blacklist', { exact: false }).count(), 0)
    assert.equal(await providerDrawer.getByText('Record key', { exact: false }).count(), 0)
    assert.equal(await providerDrawer.getByText('HH EN account', { exact: false }).count(), 0)
    assert.equal(await providerDrawer.getByText('HH RU account', { exact: false }).count(), 0)
    assert.equal(await providerDrawer.getByText('LinkedIn account', { exact: false }).count(), 0)
    await page.keyboard.press('Escape')
    await page.getByTestId('provider-clients-table').locator('tbody tr', { hasText: 'Provider Ru Visible' }).getByTestId('provider-details-button').click()
    await page.getByTestId('provider-details-drawer').waitFor()
    await assertText(page, 'HH credentials')
    await page.getByTestId('provider-hh-credential-en').getByText('Not required for this student market', { exact: false }).waitFor()
    await page.getByTestId('provider-hh-credential-ru').getByText('Ready', { exact: false }).waitFor()
    assert.equal(await page.getByTestId('provider-hh-password-ru').getAttribute('type'), 'password')
    await page.getByTestId('provider-hh-password-toggle-ru').click()
    assert.equal(await page.getByTestId('provider-hh-password-ru').getAttribute('type'), 'text')
    await page.keyboard.press('Escape')
