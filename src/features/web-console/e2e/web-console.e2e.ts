...
async function runTests(): Promise<void> {
  ...
  try {
    ...
    await page.getByTestId('profile-accordion-header').click()
    ...
    await page.getByTestId('open-dolphin-admin-button').waitFor()
    await wait(15300)
    await page.getByTestId('open-dolphin-admin-button').click()
    await page.getByTestId('dolphin-lease-panel').waitFor()
    await page.getByTestId('dolphin-lease-profiles').getByText('880000003, 880000004', { exact: false }).waitFor()
