...
const platforms = ref([])
const platformAccountPolicyEnabled = ref(false)
...
const selectedAccountPolicy = computed(() =>
  platformAccountPolicyEnabled.value ? platformAccountPolicy(selectedAccountPlatform.value) : undefined
)
const accountDisplayLabel = computed(() =>
  platformAccountPolicyEnabled.value
    ? editingAccount.value
      ? accountForm.value.accountLabel
      : selectedAccountPlatform.value
    : accountForm.value.accountLabel
)

function accountFieldEnabled(field) {
  if (!platformAccountPolicyEnabled.value) return true
  return selectedAccountPolicy.value?.fields.includes(field) ?? false
}

function accountFieldRequired(field) {
  if (!platformAccountPolicyEnabled.value) return false
  return selectedAccountPolicy.value?.requiredFields.includes(field) ?? false
}

function accountUrl(account) {
  if (platformKey(account) === 'github') return account?.linkedInUrl || account?.login || ''
  return account?.linkedInUrl || ''
}

function canEditPlatformAccount(account) {
  return !platformAccountPolicyEnabled.value || Boolean(platformAccountPolicy(account?.platform))
}
...
async function loadClientOptions() {
  ...
  platformAccountPolicyEnabled.value = result.platformAccountPolicyEnabled === true
}
...
async function logout() {
  ...
  try {
    ...
    englishLevels.value = []
    platforms.value = []
    platformAccountPolicyEnabled.value = false
    resetAccountForm()
    resetTelegramUi()
    profileEditing.value = false
    profileEditorOpen.value = ''
...
function editAccount(account) {
  ...
  accountForm.value = {
    ...
    linkedInUrl: platformAccountPolicyEnabled.value ? accountUrl(account) : account.linkedInUrl || '',
...
function accountPayload() {
  ...
  if (!platformAccountPolicyEnabled.value) {
    return {
      ...accountForm.value,
...
async function saveAccount() {
  ...
  try {
    ...
    if (!platformAccountPolicyEnabled.value && !payload.platform && !payload.platformId) {
        ...
        <Card class="accounts-card">
          ...
          <template #content>
            ...
            <form v-if="isClient && accountEditorOpen" class="account-form" data-testid="account-form" novalidate @submit.prevent="saveAccount">
              <label v-if="!editingAccount || !platformAccountPolicyEnabled" class="field">
                <span>Platform</span>
              ...
              <label class="field">
                <span>Label</span>
                <InputText v-if="platformAccountPolicyEnabled" :model-value="accountDisplayLabel" disabled data-testid="account-label" />
                <InputText v-else v-model="accountForm.accountLabel" data-testid="account-label" />
              </label>
              <label class="field">
                <span>Login</span>
              ...
              <label class="field wide-field">
                <span>{{ platformAccountPolicyEnabled ? 'URL' : 'LinkedIn URL' }}</span>
            ...
            <DataTable :value="accountRows" striped-rows responsive-layout="scroll" data-testid="accounts-table">
              ...
              <Column v-if="platformAccountPolicyEnabled" header="URL">
                <template #body="{ data }">
