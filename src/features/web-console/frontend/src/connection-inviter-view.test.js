...
import { connectionAudienceLabel, connectionCountdown, connectionFilterDiagnostics,
  connectionFunnelLabel, connectionLocalDate, connectionProgressPercent, connectionPollDelay,
  connectionQuotaLabel,
  connectionRunActive, connectionPauseFromError, connectionRunCanStart, connectionRunConfirmation,
  connectionRunFromPreviousDay, connectionRunLabel, connectionStopConfirmation,
  latestConnectionRun } from './connection-inviter-view.js'

const run = { platformAccountId: 2, status: 'succeeded', connectionCount: 320,
  localDate: connectionLocalDate(),
  ...
  counters: { sentByAudience: { recruiter: 0, technical: 0 },
    filterFunnel: { recruiter: { found: 20, structurallyValid: 19, roleMatched: 12,
...
assert.equal(connectionPollDelay(false), 15_000)
...
assert.equal(connectionPauseFromError({ status: 503,
  body: { error: 'connection_inviter_tables_missing' } }), null)
