...
import { issueTitle, issueMessage, issueResolution } from './profile-issue-view.js'
const props = defineProps({ issues: { type: Array, default: () => [] } })
const groups = computed(() => [
  { title: 'Блокеры: эти изменения нельзя отправить', severity: 'error', items: props.issues.filter(i => i.level === 'fatal') },
  ...
  <div v-if="groups.length" class="profile-issues" data-testid="profile-analysis-issues">
    <section v-for="group in groups" :key="group.title">
      ...
      <Message v-for="(issue, index) in group.items" :key="index" :severity="group.severity" :closable="false">
        ...
        <p v-if="issue.resolution">{{ issueResolution(issue) }}</p>
        <details><summary>Технические сведения: исходное сообщение</summary>
