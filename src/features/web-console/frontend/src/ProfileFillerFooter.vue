...
import { profileStage, canStopProfileGeneration } from './profile-workflow-view.js'
...
<template>
  <div class="profile-footer" data-testid="profile-filler-footer">
    <div class="profile-footer-note">
      ...
      <Button v-if="filler.job.value && !filler.busy.value" label="Подготовить заново"
    ...
    <div class="profile-footer-buttons">
      <Button v-if="canStopProfileGeneration(filler.trackedJob.value ?? filler.job.value)"
        label="Закрыть и остановить" severity="danger" outlined :loading="filler.pending.value"
        :disabled="filler.pending.value || (filler.trackedJob.value ?? filler.job.value)?.phase === 'stopping_generation'"
        data-testid="profile-generation-stop" @click="filler.stopGeneration" />
      ...
      <template v-else-if="filler.job.value.status === 'preview_ready'">
        <Button v-if="filler.manualDirty.value && !filler.job.value.preview?.generation" label="Пересобрать Preview"
      ...
      <Button v-else-if="filler.job.value.status === 'waiting_retry' || filler.job.value.previewRecovery"
        :label="filler.job.value.previewRecovery === 'save' ? 'Повторить сохранение Preview' :
          filler.job.value.previewRecovery === 'rebuild' ? 'Восстановить Preview без генерации' : 'Продолжить подготовку'"
    ...
    <small v-if="filler.job.value?.previewRecovery">Повторная генерация OpenAI и запись в LinkedIn не запускаются.</small>
