...
const ORCHESTRATOR_CLIENT_REPORT_TIMEOUT_MS = Number(
  process.env.ORCHESTRATOR_CLIENT_REPORT_TIMEOUT_MS ?? 30000
)
const DEFAULT_HH_SCENARIO_AUTH_MAX_RECHECKS = 3
const parsedScenarioAuthMaxRechecks = Number(
  process.env.HH_SCENARIO_AUTH_MAX_RECHECKS ?? DEFAULT_HH_SCENARIO_AUTH_MAX_RECHECKS
)
const HH_SCENARIO_AUTH_MAX_RECHECKS = Number.isFinite(
  parsedScenarioAuthMaxRechecks
)
  ? Math.max(1, Math.floor(parsedScenarioAuthMaxRechecks))
  : DEFAULT_HH_SCENARIO_AUTH_MAX_RECHECKS
const HH_SCENARIO_EARLY_AUTH_CHECK_MS = Number(
  process.env.HH_SCENARIO_EARLY_AUTH_CHECK_MS ?? 30000
)
...
module.exports = {
  ...
  DOLPHIN_PREFLIGHT_AUTO_CLEANUP: parseBooleanEnv(
    process.env.DOLPHIN_PREFLIGHT_AUTO_CLEANUP,
    true
  ),
  ...
  DOLPHIN_PROFILE_STOP_VERIFY_INTERVAL_MS: Number(
    process.env.DOLPHIN_PROFILE_STOP_VERIFY_INTERVAL_MS ?? 1000
  ),
  DOLPHIN_PROFILE_STOP_VERIFY_MS: Number(
    process.env.DOLPHIN_PROFILE_STOP_VERIFY_MS ?? 10000
  ),
  ...
  INDEX_SCRIPT_PATH: path.resolve(__dirname, '../browser-responder/index.js'),
  LOCAL_RUN_ID,
  LOCAL_RUN_LOG_DIR,
