...
const { loadPlaywright } = require('../../../platform/browser/playwright.ts')
...
async function openScenarioAndInjectIndex(
  ...
  const earlyAuth = earlyAuthBeforeStart

  if (
    earlyAuth &&
    ['logged_out', 'captcha'].includes(earlyAuth.state)
  ) {
    options.logLifecycleEvent?.(
