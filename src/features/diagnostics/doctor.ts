...
type AppDb = import('../../platform/db/types.ts').AppDb
type ClientAutomationData = import('../../platform/db/types.ts').ClientAutomationData
type ClientHHAuthCredentials =
  import('../../platform/db/types.ts').ClientHHAuthCredentials
...
const { parseMarketEnv } = require('../hh-responses/orchestrator/config.ts') as {
  parseMarketEnv(value: string | undefined): 'Ru' | 'En'
}
const {
  getConfiguredClientIds,
  getConfiguredClientNames
} = require('../hh-responses/orchestrator/clients.ts') as {
...
const { assertDolphinAppRunning } = require('../../integrations/dolphin/index.ts') as {
