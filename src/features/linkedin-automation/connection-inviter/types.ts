import type { ConnectionMarketTier, ConnectionSearchTemplate, SearchAudience } from './catalog.ts'
...
export type ConnectionSearchStreamState = {
  ...
  city?: string
  locationId?: string
  marketTier?: ConnectionMarketTier
  termIndex?: number
  term?: string
  emptyCursorStreak?: number
  page: number
  nextCursor?: string
}
...
export type ConnectionSearchProgress = {
  ...
  eligible: number
  skipped: number
  consecutiveEmptyRecruiterSearches: number
  exhausted: Record<SearchAudience, boolean>
  pass: number
  passUsedSearchKeys: string[]
...
export type ConnectionRunCounters = {
  ...
  shortfallByAudience?: Record<SearchAudience, number>
...
export type ConnectionInviterStore = {
  ...
  findHistory(accountId: string, personId: string, fresh?: boolean):
    Promise<ConnectionHistoryItem | undefined>
  ...
  recordRetry?(): void
  runWithNocoBudget?<T>(run: ConnectionRun, action: () => Promise<T>): Promise<T>
