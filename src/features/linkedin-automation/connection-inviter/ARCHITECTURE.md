...
3. Отдельного недельного provider safety budget превышать нельзя.
4. Недельной очереди и отдельного недельного списка нет.
5. Используется одна постоянная история за всё время, а не история по неделям.
6. Повторное приглашение той же паре `accountId + personId` запрещено.
7. При пропущенном запуске система не догоняет накопившуюся пачку.
8. Все отправки одного аккаунта выполняются последовательно.
9. Неопределённый результат отправки никогда не повторяется вслепую.
...
- `relation.new` — если событие реально доступно для tenant/provider, запустить
  reconciliation; не считать событие единственным доказательством принятия.
...
Доступность `relation.new` для LinkedIn расходится между текущими страницами
официальной документации Unipile. Поэтому обязательный fallback — безопасная
сверка sent invitations/relations с jitter и без частого фиксированного polling.
...
Общие правила: [account lifecycle](../account-connection/docs/ACCOUNT_LIFECYCLE.md),
[data boundaries](../core/storage/DATA_BOUNDARIES.md) и
[LinkedIn limits](../core/safety/LINKEDIN_LIMITS.md).
