...
async function staleRunClosesWithoutCarryover() {
  ...
  for (let index = 0; index < 13; index += 1) {
    const personId = `confirmed-old-${index}`
    await test.store.claimHistory({ historyKey: `acc_test:${personId}`, runId: run.runId,
      platformAccountId: 7, accountId: 'acc_test', personId,
      audience: index < 9 ? 'recruiter' : 'technical', searchKey: 'old-confirmed',
      name: `Confirmed ${index}`, headline: 'Recruiter', location: 'Berlin', status: 'sent',
      reasonCode: 'pending_readback_confirmed', discoveredAt: old.toISOString(),
      updatedAt: old.toISOString(), sentAt: old.toISOString(), verifiedAt: old.toISOString() })
  }
  const service = createConnectionInviterService({ ...test, now: () => now, autoRecover: false })
  await service.recover()
...
async function malformedOpenHistoryFailsClosed() {
  ...
  const run = makeRun({ platformAccountId: 7, clientId: 3, clientName: 'Test',
    ...
    stackId: 10, stack: 'GO' }, now, 'Europe/Moscow', false)
  run.stage = 'sending'; await test.store.createRun(run)
  test.store.listOpenHistory = async () => {
    throw Object.assign(new Error('Malformed Noco records response.'), {
      code: 'noco_response_invalid', details: { httpStatus: 503 }
    })
  }
  let releaseRetry!: () => void; let signalRetry!: () => void
  const retryStarted = new Promise<void>(resolve => { signalRetry = resolve })
  const retryWait = new Promise<void>(resolve => { releaseRetry = resolve })
  const service = createConnectionInviterService({ ...test, now: () => now,
    autoRecover: false, sleep: async () => { signalRetry(); await retryWait },
    logger: { event() {} } })
  const recovery = service.recover(); await retryStarted; service.stop(); releaseRetry(); await recovery
  const unchanged: any = await test.store.getRun(run.runId)
  assert.equal(unchanged.status, 'running')
  assert.equal(['sending', 'recovering', 'waiting_retry'].includes(unchanged.stage), true)
  assert.equal(test.metrics.sends, 0)
}

async function startupRecoveryRetriesNocoBeforeInvitations() {
  const test = fixture({ stack: 'GO', connectionCount: 149 })
  const run = makeRun({ platformAccountId: 7, clientId: 3, clientName: 'Test',
    linkedinUrl: 'https://www.linkedin.com/in/test-client/', accountId: 'acc_test',
    stackId: 10, stack: 'GO' }, now, 'Europe/Moscow', false)
  run.stage = 'sending'
  await test.store.createRun(run)
  const recoveredPersonId = 'startup-recovery-sending'
  await test.store.claimHistory({ historyKey: `acc_test:${recoveredPersonId}`, runId: run.runId,
    platformAccountId: 7, accountId: 'acc_test', personId: recoveredPersonId,
    audience: 'recruiter', searchKey: 'startup-recovery', name: 'Recovered Person',
    headline: 'Technical Recruiter', location: 'Berlin', status: 'sending',
    reasonCode: 'invitation_claimed', discoveredAt: now.toISOString(), updatedAt: now.toISOString() })

  const order: string[] = []
  const retryDelays: number[] = []
  const recoveryLogs: Array<{ stage: string; status: string; details?: any }> = []
  const listRuns = test.store.listRuns.bind(test.store)
  let listRunsAttempts = 0
  test.store.listRuns = async (limit?: number) => {
    listRunsAttempts += 1; order.push(`list_runs_${listRunsAttempts}`)
    if (listRunsAttempts === 1) {
      throw Object.assign(new Error('startup listRuns rate limited'), {
  ...
  const listOpenHistory = test.store.listOpenHistory.bind(test.store)
  let openHistoryReadBack = false
  test.store.listOpenHistory = async (platformAccountId: number, limit?: number) => {
    order.push('open_history_readback_started')
    const value = await listOpenHistory(platformAccountId, limit)
    openHistoryReadBack = true; order.push('open_history_readback_finished')
    return value
  }

  const listPendingInvitations = test.adapter.listPendingInvitations.bind(test.adapter)
  test.adapter.listPendingInvitations = async (accountId: string, offset = 0) => {
    const value = await listPendingInvitations(accountId, offset)
    return offset ? value : { ...value,
      items: [{ user_id: recoveredPersonId }, ...(value.items ?? [])] }
  }
  const sendInvitation = test.adapter.sendInvitation.bind(test.adapter)
  test.adapter.sendInvitation = async (accountId: string, personId: string) => {
    order.push(`invitation_post:${personId}`)
    assert.equal(openHistoryReadBack, true,
      'Startup recovery reached invitation POST before open-history read-back.')
  ...
  const service = createConnectionInviterService({ ...test,
    ...
    logger: { event(stage: string, status: string, details?: any) {
      recoveryLogs.push({ stage, status, details })
    } }
  })
  const completed: any = await waitRun(service, run.runId)
  const history: any[] = await service.history(7)
  ...
  assert.equal(listRunsAttempts, 2)
  assert.equal(retryDelays.slice(0, 90).reduce((total, value) => total + value, 0), 90_000)
  assert.equal(retryDelays.slice(0, 90).every(value => value <= 1_000), true)
  assert.equal(completed.status, 'succeeded')
  assert.equal(completed.counters.sent, 5)
  assert.equal(test.metrics.sends, 4)
  assert.equal(history.find(item => item.personId === recoveredPersonId)?.status, 'sent')
  assert.equal(order.indexOf('open_history_readback_finished') <
    order.findIndex(item => item.startsWith('invitation_post:')), true)
  assert.equal(recoveryLogs.some(item => item.stage === 'run_recovery_retry' &&
    item.status === 'failed' && item.details?.attempt === 1), true)
  assert.equal(recoveryLogs.some(item => item.stage === 'run_recovery_retry' &&
    item.status === 'succeeded' && item.details?.attempt === 1), true)
}

async function stopCancelsStartupRecoveryBackoff() {
  const test = fixture({ stack: 'GO', connectionCount: 149 })
  let listRunsAttempts = 0
  test.store.listRuns = async () => {
    listRunsAttempts += 1
    throw Object.assign(new Error('persistent startup rate limit'), {
      code: 'noco_rate_limited', details: { httpStatus: 429 }
    })
  }
  ...
  assert.equal(requestedSlices[0], 1_000)
  service.stop()
...
async function stoppedRecoveryCannotMutateAfterFenceHandoff() {
  const test = fixture({ stack: 'GO', connectionCount: 149 })
  const run = makeRun({ platformAccountId: 7, clientId: 3, clientName: 'Test',
    linkedinUrl: 'https://www.linkedin.com/in/test-client/', accountId: 'acc_test',
    stackId: 10, stack: 'GO' }, now, 'Europe/Moscow', false)
  run.stage = 'waiting_retry'; await test.store.createRun(run)
  const listRuns = test.store.listRuns.bind(test.store)
  let releaseList!: () => void; let signalListStarted!: () => void
  ...
  test.store.listRuns = async (limit?: number) => {
    signalListStarted(); await heldList; return listRuns(limit)
  }
...
async function run() {
  const cases: Array<[string, () => Promise<unknown>]> = [
    ['timer', timerRecovery], ['sending', sendingRecovery], ['secondWriter', secondWriter],
  ...
  for (const [name, test] of cases) {
    console.log(`recovery case: ${name}`); await test()
  }
}
