import { CONNECTION_SEARCH_CATALOG, connectionSearchTerms } from '../catalog.ts'
...
export function fixture(options: { stack?: string; sendFailure?: any; pendingReadFailure?: any;
  ...
  const adapter: any = {
    ...
    async searchPeople(_accountId: string, input: { keywords: string; locationId: string }) {
      reads += 1
      const template = CONNECTION_SEARCH_CATALOG.find(item =>
        connectionSearchTerms(item, currentStack, !currentStack).includes(input.keywords) &&
      ...
      return { items: Array.from({ length: 4 }, (_value, index) => { person += 1
        ...
        return {
          ...
          headline: template.audience === 'recruiter' ? 'Technical Recruiter' :
            `${currentStack} Software Engineer`, location: `${template.city}, Region`,
          network_distance: 2
        }
      }) }
    },
