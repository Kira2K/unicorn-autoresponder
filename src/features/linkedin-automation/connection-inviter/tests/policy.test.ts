...
const { networkDistance, profileAllowsInvitation, profileIsConnected } = require('../relation-policy.ts') as
...
assert.equal(profileIsConnected({ relationship: 'not_connected' }), false)
assert.equal(profileIsConnected({ relationship: 'connected' }), true)
assert.deepEqual(profileAllowsInvitation({ relationship: 'not_connected', network_distance: 2 }),
