...
export function createConnectionTableResolver(client: any) {
  let cache: Record<string, string> | undefined
  return async () => {
    if (cache) return cache
    const endpoint = `/api/v2/meta/bases/${client.config.baseId}/tables`
    const tables = await client.request('get', endpoint)
    ...
    for (const [key, definition] of Object.entries(CONNECTION_TABLES)) {
      ...
      const missing = missingUniqueColumns(await client.fetchTableMeta(table.id), definition.columns)
