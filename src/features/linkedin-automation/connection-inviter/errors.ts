...
export function connectionErrorCode(error: unknown): string {
  ...
  return /^(connection_|linkedin_|unipile_|noco_|automation_)/.test(code) ? code.slice(0, 120) :
...
export function transientConnectionError(error: any): boolean {
  ...
  return isAutomationPause(code) || ['connection_storage_read_unavailable', 'noco_rate_limited', 'noco_timeout', 'noco_unreachable', 'noco_service_unavailable',
