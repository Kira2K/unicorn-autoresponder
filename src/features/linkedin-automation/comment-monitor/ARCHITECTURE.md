...
Для runtime-состояния требуется отдельная долговечная БД приложения.
PostgreSQL — подходящий кандидат, но конкретная СУБД, подключение и миграции
согласуются отдельной задачей.
...
Worker работает с целевым часовым интервалом и server-side jitter, а не в
фиксированную минуту каждого часа:
