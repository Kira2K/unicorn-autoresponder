...
function parseYearMonth(
  ...
  if (isObject(value)) {
    ...
    addWarning(issues, path, 'Ожидалась дата {year, month} с корректным месяцем.', 'Поле проигнорировано.')
    return undefined
  }
  ...
  if (!match) {
    if (value !== undefined && value !== null && value !== '') {
      addWarning(issues, path, 'Ожидалась дата в формате YYYY-MM или {year, month}.', 'Поле проигнорировано.')
    }
    return undefined
  }
