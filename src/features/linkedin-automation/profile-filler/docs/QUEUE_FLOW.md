...
flowchart TD
    ...
    RO --> C["Validate and normalize profile.json"]
    C --> B["Read required current profile sections"]
    B --> BS["Save immutable snapshot and section statuses"]
    BS --> D["Compare saved state with desired state"]
    ...
    I -->|"Yes"| J["Create mutation job and verify plan plus snapshot binding"]
