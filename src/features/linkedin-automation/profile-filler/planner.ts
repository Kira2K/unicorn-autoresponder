...
type PlannerOptions = {
  ...
  sectionStatuses?: Partial<Record<import('./types.ts').PlanSection, SectionReadStatus>>
  sourceSnapshot?: ProfileSnapshotReference
}

function queueOrder(step: PlanStep): number {
  if (step.section === 'headline') return 10
  if (step.section === 'about') return 20
  if (step.section === 'experience' && step.action === 'update') return 30
  if (step.section === 'education' && step.action === 'update') return 40
  if (step.section === 'skills') return 50
  if (step.section === 'experience') return 60
  if (step.section === 'education') return 70
  return 80
}
...
function unavailableReadStatus(
  options: PlannerOptions,
  section: import('./types.ts').PlanSection,
): SectionReadStatus | undefined {
  const status = options.sectionStatuses?.[section]
  return status && status !== 'complete' && status !== 'empty' ? status : undefined
}

function unavailableReadWarning(
  section: import('./types.ts').PlanSection,
  status: SectionReadStatus,
): ValidationIssue {
  return warning(
    `profile.${section}`,
    `Раздел ${section} не подтверждён полным чтением: ${status}.`,
    'Раздел пропущен; create/update разрешены только после статуса complete или empty.',
  )
}
...
async function buildProfilePlan(
  ...
  const headlineUnavailable = unavailableReadStatus(options, 'headline')
  if (desired.headline !== undefined && headlineUnavailable) {
    issues.push(unavailableReadWarning('headline', headlineUnavailable))
  } else if (desired.headline !== undefined && desired.headline !== String(currentProfile.description ?? '')) {
    steps.push({
      id: 'headline',
      section: 'headline',
      action: 'update',
  ...
  const aboutUnavailable = unavailableReadStatus(options, 'about')
  if (desired.about !== undefined && aboutUnavailable) {
    issues.push(unavailableReadWarning('about', aboutUnavailable))
  } else if (desired.about !== undefined && desired.about !== String(currentProfile.bio ?? '')) {
    steps.push({
      id: 'about',
      section: 'about',
      action: 'update',
  ...
  const experienceUnavailable = unavailableReadStatus(options, 'experience')
  if (experienceUnavailable) {
    issues.push(unavailableReadWarning('experience', experienceUnavailable))
  } else if (throttled.has('linkedin_experience')) {
  ...
  const educationUnavailable = unavailableReadStatus(options, 'education')
  if (educationUnavailable) {
    issues.push(unavailableReadWarning('education', educationUnavailable))
  } else if (throttled.has('linkedin_education')) {
  ...
  const skillsUnavailable = unavailableReadStatus(options, 'skills')
  if (skillsUnavailable) {
    issues.push(unavailableReadWarning('skills', skillsUnavailable))
  } else if (throttled.has('linkedin_skills')) {
  ...
  const openToWorkUnavailable = unavailableReadStatus(options, 'open_to_work')
  let openToWorkPayload: JsonObject | undefined
  if (desired.openToWork && openToWorkUnavailable) {
    issues.push(unavailableReadWarning('open_to_work', openToWorkUnavailable))
  } else {
    openToWorkPayload = await resolveOpenToWork(desired, options.resolveParameter, issues)
  }
  if (openToWorkPayload) {
    steps.push({
      ...
      verification: { kind: 'open_to_work', expected: openToWorkPayload }
    })
  }

  steps.sort((left, right) => queueOrder(left) - queueOrder(right))
  return {
    account,
    identity,
