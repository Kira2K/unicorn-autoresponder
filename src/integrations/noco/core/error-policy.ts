...
function header(error: any, name: string): string {
  const headers = error?.response?.headers
  return String(headers?.get?.(name) ?? headers?.[name] ?? '').trim()
}

export function nocoRetryAfterMs(error: any): number | undefined {
...
export function isRetryableNocoRequest(error: any, method: string): boolean {
  const code = nocoErrorCode(error)
  if (!code || method === 'post') return false
  return ['noco_rate_limited', 'noco_timeout', 'noco_unreachable',
