...
const { ensureLinkedInProfileJobsTable } = require('./logic.ts') as {
  ensureLinkedInProfileJobsTable(client: any, baseId: string, apply: boolean): Promise<unknown>
}
...
async function main() {
  ...
  console.log(JSON.stringify(
    await ensureLinkedInProfileJobsTable(client, client.config.baseId, args.apply), null, 2))
}
