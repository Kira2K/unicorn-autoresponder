...
хранятся отдельно от исходной схемы Noco. Импорт сохраняет их у оставшихся записей.
`backend/postgres/workflow-schema.mts` проверяет наличие полей; добавление требует
явного разрешения. При старте приложения схема не меняется. Правила статусов,
