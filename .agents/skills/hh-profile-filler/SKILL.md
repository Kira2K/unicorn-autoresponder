...
description: Fill and replace HH resumes in Dolphin from final Google Drive CVs and read-only application data (PostgreSQL or Noco). Use when Codex is asked to prepare, dry-run, run, schedule, supervise, diagnose, or report HH profile filling by client/status/market, including About, privacy, employer stop-lists, resume duplicates, and password login.
---
...
- All modes use this choice: preparation, filling, recovery, scan/pending and live smoke. A SQL failure must not fall back to Noco.
